...
function _set_builder_language {
	...
	echo "Lingua builder" $file;

	OLD_IFS=$IFS;
	IFS=$(echo -e '\n\t');					#guarda se mettere a local funziona per tutto il builder poi
	OUTPUT=($(cat $file));					#guarda se mettere a local funziona per tutto il builder poi
