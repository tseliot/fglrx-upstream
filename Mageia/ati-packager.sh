...
#
# Purpose
#   Mageia ATI packaging script
#
# Usage
#   See README.distro document
#
# License
#   This file is available with the same license as Mageia fglrx.spec,
...
buildCheck()
{
    ...
    if [ ! -f /etc/mageia-release ]; then
        echo "You can build Mageia packages only on a Mageia system."
        return 1
    fi
...
commonRPMparams()
{
    ...
    echo -e "--define\n _sourcedir $PWD/packages/Mageia"
}
...
buildPackage()
{
	...
	--define "_sourcedir ${installer_root}/packages/Mageia" \
	--define "ati_dir ${installer_root}" \
	${installer_root}/packages/Mageia/fglrx.spec &> $temp_root/output.log
...
installPackage()
{
    ...
    if [ "${package}" != "${distrover}" ]; then
        echo "Mageia ${distrover} can't use ${package} packages."
        exit 1
    fi
...
--identify)
    package=$2
    if [ -f /etc/mageia-release ] && [ "${package}" = "$(cut -d" " -f1 /etc/version)" ]; then
        exit 0
    fi
    exit ${ATI_INSTALLER_ERR_VERS}
    ;;
--get-maintainer)
    echo "Anssi Hannula <anssi@mageia.org>"
    exit 0
    ;;
--getAPIVersion)
    exit 2
